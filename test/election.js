...
contract("Election", function(accounts) {
  ...
  it("it initializes the candidates with the correct values", function() {
    return Election.deployed().then(function(instance) {
      ...
      assert.equal(candidate[3], "Bareilly", "contains the correct constituency");
      ...
      assert.equal(candidate[3], "Jhansi", "contains the correct constituency");
      assert.equal(candidate[4], 0, "contains the correct votes count");
      return electionInstance.candidates(3);
    }).then(function(candidate) {
      assert.equal(candidate[0], 4, "contains the correct id");
      assert.equal(candidate[1], "Mamta Banerjee", "contains the correct name");
      assert.equal(candidate[2], "All India Trinamool Congress", "contains the correct party");
      assert.equal(candidate[3], "Badaun", "contains the correct constituency");
      assert.equal(candidate[4], 0, "contains the correct votes count");
      return electionInstance.candidates(4);
    }).then(function(candidate) {
      assert.equal(candidate[0], 5, "contains the correct id");
      assert.equal(candidate[1], "Mayavati", "contains the correct name");
      assert.equal(candidate[2], "Bahujan Samaj Party", "contains the correct party");
      assert.equal(candidate[3], "Rampur", "contains the correct constituency");
      assert.equal(candidate[4], 0, "contains the correct votes count");
    });
  });
